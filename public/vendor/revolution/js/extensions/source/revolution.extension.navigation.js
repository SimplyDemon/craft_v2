...
(function( $ ) {
    ...
    jQuery.extend( true, _R, {
        ...
        hideUnHideNav: function( opt ) {
            ...
            if ( ckNO( c ) ) {
                biggerNav( opt.c.parent().find( '.tp-thumbs' ), c.hide_under, w, c.hide_over );
            }
            if ( ckNO( d ) ) {
        ...
        createNavigation: function( container, opt ) {
            ...
            if ( c ) {
                swipeAction( cp.find( '.tp-thumbs' ), opt );
            }
            if ( d ) {
                swipeAction( cp.find( '.tp-tabs' ), opt );
            }
    ...
    var moveThumbsInPosition = function( container, opt ) {
        ...
        if ( thumbdir !== "vertical" && t.width() >= el.width() ) {
            curpos = 0;
        }
        if ( thumbdir === "vertical" && t.height() >= el.height() ) {
            curpos = 0;
        }
        ...
        if ( !container.hasClass( "dragged" ) ) {
            if ( thumbdir === "vertical" ) {
                el.data( 'tmmove', punchgs.TweenLite.to( el, 0.5, {
                    top: curpos + "px",
    ...
    var initMouseScroll = function( container, opt ) {
        ...
        if ( opt.navigation.mouseScrollNavigation !== "on" && opt.navigation.mouseScrollNavigation !== "carousel" ) {
            return;
        }
    ...
    var swipeAction = function( container, opt, vertical ) {
        ...
        if ( !jQuery.fn.swipetp ) {
            jQuery.fn.swipetp = jQuery.fn.swipe;
        }
        ...
        SwipeOn.swipetp( {
            ...
            swipeStatus: function( event, phase, direction, distance, duration, fingerCount, fingerData ) {
                ...
                if ( (
                    ...
                    if ( withinthumbs > 0 ) {
                        opt.dragStartedOverThumbs = true;
                    }
                    if ( withintabs > 0 ) {
                        opt.dragStartedOverTabs = true;
                    }
    ...
    var callAllDelayedCalls = function( container, opt ) {
        ...
        if ( ckNOLO( opt.navigation.arrows ) ) {
            punchgs.TweenLite.delayedCall( _ISM ? opt.navigation.arrows.hide_delay_mobile : opt.navigation.arrows.hide_delay, showHideNavElements, [
                cp.find( '.tparrows' ),
                opt.navigation.arrows,
                "hide"
            ] );
        }

        if ( ckNOLO( opt.navigation.bullets ) ) {
            punchgs.TweenLite.delayedCall( _ISM ? opt.navigation.bullets.hide_delay_mobile : opt.navigation.bullets.hide_delay, showHideNavElements, [
                cp.find( '.tp-bullets' ),
        ...
        if ( ckNOLO( opt.navigation.thumbnails ) ) {
            punchgs.TweenLite.delayedCall( _ISM ? opt.navigation.thumbnails.hide_delay_mobile : opt.navigation.thumbnails.hide_delay, showHideNavElements, [
                cp.find( '.tp-thumbs' ),
        ...
        if ( ckNOLO( opt.navigation.tabs ) ) {
            punchgs.TweenLite.delayedCall( _ISM ? opt.navigation.tabs.hide_delay_mobile : opt.navigation.tabs.hide_delay, showHideNavElements, [
                cp.find( '.tp-tabs' ),
                opt.navigation.tabs,
                "hide"
            ] );
    ...
    var addBullet = function( container, o, li, opt ) {
        ...
        //bgimage = li.data('thumb') !==undefined ? li.data('thumb') : li.find('.defaultimg').data('lazyload')
        // !==undefined && li.find('.defaultimg').data('lazyload') !== 'undefined' ?
        // li.find('.defaultimg').data('lazyload') : li.find('.defaultimg').data('src');

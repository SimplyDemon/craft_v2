...
(function( jQuery, undefined ) {
    ...
    jQuery.fn.extend( {
        ...
        revolution: function( options ) {
            ...
            var defaults = {
                delay: 9000,
                responsiveLevels: 4064,					// Single or Array for Responsive Levels i.e.: 4064 or i.e. [2048, 1024,
                                            // 768, 480]
                visibilityLevels: [2048, 1024, 778, 480],	// Single or Array for Responsive Visibility Levels i.e.:
                                                          // 4064 or i.e. [2048, 1024, 768, 480]
                ...
                fullScreenOffsetContainer: "",			// Size for FullScreen Slider minimising Calculated on the Container
                                                 // sizes
                fullScreenOffset: "0",					// Size for FullScreen Slider minimising

                hideCaptionAtLimit: 0,					// It Defines if a caption should be shown under a Screen Resolution ( Basod
                                           // on The Width of Browser)
                hideAllCaptionAtLimit: 0,				// Hide all The Captions if Width of Browser is less then this value
                hideSliderAtLimit: 0,					// Hide the whole slider, and stop also functions if Width of Browser is less
                                          // than this value
                disableProgressBar: "off",				// Hides Progress Bar if is set to "on"
                stopAtSlide: - 1,							// Stop Timer if Slide "x" has been Reached. If stopAfterLoops set to 0, then
                                        // it stops already in the first Loop at slide X which defined. -1 means do not
                                        // stop at any slide. stopAfterLoops has no sinn in this case.
                stopAfterLoops: - 1,						// Stop Timer if All slides has been played "x" times. IT will stop at THe
                                          // slide which is defined via stopAtSlide:x, if set to -1 slide never stop
                                          // automatic
                ...
                navigation: {
                    ...
                    keyboard_direction: "horizontal",		//	horizontal - left/right arrows,  vertical - top/bottom
                                                       // arrows
                    ...
                    touch: {
                        ...
                        swipe_treshold: 75,					// The number of pixels that the user must move their finger by before
                                                // it is considered a swipe.
                ...
                if ( container != undefined && container.length > 0 && jQuery( 'body' ).find( '#' + container.attr( 'id' ) ).length > 0 ) {
                    ...
                    if ( opt && opt.li.length > 0 ) {
                        if ( sindex > 0 || sindex <= opt.li.length ) {
                            ...
                            if ( _R.updateNavIndexes ) {
                                _R.updateNavIndexes( opt );
                            }
                            if ( nextslideafter ) {
                                container.revnext();
                            }
        ...
        revkill: function( oy ) {
            ...
            if ( container != undefined && container.length > 0 && jQuery( 'body' ).find( '#' + container.attr( 'id' ) ).length > 0 ) {
                ...
                punchgs.TweenLite.killTweensOf( container, false );
                bt.remove();
                try {container.closest( '.forcefullwidth_wrapper_tp_banner' ).remove();}
                catch ( e ) {}
                try {container.closest( '.rev_slider_wrapper' ).remove()}
                catch ( e ) {}
    ...
    var getNeededScripts = function( o, c ) {
        ...
        if ( !c.data( 'version' ) || !c.data( 'version' ).toString().match( /5./gi ) ) {
            ...
            c.find( 'img' ).each( function() {
                if ( jQuery( this ).data( 'kenburns' ) == "on" ) {
                    n.kenburns = true;
                }
            } );
    ...
    var prepareOptions = function( container, opt ) {
        opt.carousel.maxVisibleItems = opt.carousel.maxVisibleItems < 1 ? 999 : opt.carousel.maxVisibleItems; // === 1
                                                                                                              // ? 2 :
    ...
    var initSlider = function( container, opt ) {
        ...
        opt.ul.find( '>li' ).each( function( i ) {
            var li = jQuery( this );
            if ( li.data( 'hideslideonmobile' ) == "on" && _ISM ) {
                li.remove();
            }
    ...
    var runSlider = function( container, opt ) {
        ...
        if ( !container.hasClass( "revslider-initialised" ) ) {
            ...
            // REMOVE ANY VIDEO JS SETTINGS OF THE VIDEO  IF NEEDED  (OLD FALL BACK, AND HELP FOR 3THD PARTY PLUGIN
            // CONFLICTS)
            ...
            if ( deeplink.length < 9 ) {
                if ( deeplink.split( 'slide' ).length > 1 ) {
                    ...
                    if ( dslide > opt.slideamount ) {
                        dslide = opt.slideamount;
                    }
    ...
    var containerResized = function( c, opt ) {
        ...
        if ( !_R.resizeThumbsTabs || _R.resizeThumbsTabs( opt ) === true ) {
            ...
            // DOUBLE CALL FOR SOME FUNCTION TO AVOID PORTRAIT/LANDSCAPE ISSUES, AND TO AVOID FULLSCREEN/NORMAL SWAP
            // ISSUES
            if ( _R.animateTheCaptions ) {
    ...
    var swapSlideProgress = function( opt, defimg, container ) {
        ...
        if ( opt.sc_indicator == "arrow" ) {
            if ( ai == 0 && ni == opt.slideamount - 1 ) {
                opt.sdir = 1;
            }
            if ( ai == opt.slideamount - 1 && ni == 0 ) {
                opt.sdir = 0;
            }
        }
        ...
        if ( actli.index() != nextli.index() && opt.firststart != 1 ) {
            if ( _R.removeTheCaptions ) {
                _R.removeTheCaptions( actli, opt );
            }
        }
    ...
    var removeAllListeners = function( container, opt ) {
        container.children().each( function() {
            try { jQuery( this ).die( 'click' ); }
            catch ( e ) {}
            try { jQuery( this ).die( 'mouseenter' );}
            catch ( e ) {}
            try { jQuery( this ).die( 'mouseleave' );}
            catch ( e ) {}
            try { jQuery( this ).unbind( 'hover' );}
            catch ( e ) {}
        } )
        try { container.die( 'click', 'mouseenter', 'mouseleave' );}
        catch ( e ) {}
    ...
    var restartOnFocus = function( opt ) {
        if ( opt == undefined || opt.c == undefined ) {
            return false;
        }

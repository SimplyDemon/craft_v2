...
-------------------------------------------------------------------*/
        var options = {
            ...
            templates: {
                ...
                countdown: '<div><span>%H:%M:%S</span></div>'
            },
...
-------------------------------------------------------------------*/
        ...
        function runParallaxMouse( x, y ) {
            ...
            $parallaxMouseList.each( function eachParallaxMouse() {
                ...
                // animate
                // yep, magic number just to let user add attribute data-mouse-parallax="4" instead of
                // data-mouse-parallax="20"
...
-------------------------------------------------------------------*/
        function initLinkEffects() {
            ...
            (0, _utility.$)( '.nk-navbar:not(.nk-navbar-no-link-effect) ul > li > a:not(.btn):not(.nk-btn):not(.no-link-effect)' );
                ...
                var validator = $form.validate( {
                    ...
                    groups: function groups() {
                        ...
                        $form.find( '.input-group' ).each( function() {
                            var inputs = (0, _utility.$)( this ).find( 'input:text:not(:hidden)' ); // TODO: What about
                                                                                                    // non-text inputs
                                                                                                    // like number?
                            if ( inputs.length > 1 ) {
                        ...
                        _utility.$.ajax( {
                            ...
                            success: function success( resp ) {
                                ...
                                if ( resp.result === 'success' ) {
                                    ...
                                    // If the form has errors, display them, inline if possible, or appended to
                                    // #mce-error-response
                                } else {
                                    // Example errors - Note: You only get one back at a time even if you submit
                                    // several that are bad. Error structure - number indicates the index of the merge
                                    // field that was invalid, then details Object {result: "error", msg: "6 - Please
                                    // enter the date"} Object {result: "error", msg: "4 - Please enter a value"}
                                    ...
                                    try {
                                        // If index is -1 if means we don't have data on specifically which field was
                                        // invalid. Just lump the error message into the generic response div.
                                        if ( index === - 1 ) {
        ...
        function initPluginStickySidebar() {
            ...
            (0, _utility.$)( '.nk-sidebar-sticky' ).each( function eachStickySidebar() {
                ...
                $this.wrapInner( '<div>' ).children().stick_in_parent( {
                    ...
                    // we need to set min height on parent block (in theme it is equal height column) to prevent
                    // sidebar content jumping
        ...
        function initPluginCountdown() {
            ...
            (0, _utility.$)( '.nk-countdown' ).each( function eachCountdown() {
                ...
                (0, _utility.$)( this ).countdown( end, function onCountdownTick( event ) {
                    ...
                    (0, _utility.$)( this ).html( event.strftime( '<div><span>' + totalHours + ':%M:%S</span></div>' ) );
                } );
            } );
        }

        ...
        @if ($categories)
            <ul class="list-group">
                @foreach($categories as $single)
                    ...
                    <div class="nk-accordion" id="accordion-category-{{$single->id}}" role="tablist" aria-multiselectable="true">
                        <div class="panel panel-default">
                            <div class="panel-heading" role="tab" id="category-{{$single->id}}-heading">
                                <a class="collapsed" data-toggle="collapse" data-parent="#accordion-category-{{$single->id}}" href="#category-{{$single->id}}" aria-expanded="false" aria-controls="category-{{$single->id}}">
                                    {{$single->name}}
                                </a>
                            </div>
                                    ...
                                    @if($single->subCategories)
                                        <ul class="list-group">
                                            @foreach( $single->subCategories as $subCategory)
                                                <div class="nk-accordion" id="accordion-category-{{$subCategory->id}}" role="tablist" aria-multiselectable="true">
                                                    <div class="panel panel-default">
                                                        <div class="panel-heading" role="tab" id="category-{{$subCategory->id}}-heading">
                                                            <a class="collapsed" data-toggle="collapse" data-parent="#accordion-category-{{$subCategory->id}}" href="#category-{{$subCategory->id}}" aria-expanded="false" aria-controls="category-{{$subCategory->id}}">
                                                                {{$subCategory->name}}
                                                            </a>
                                                        </div>
